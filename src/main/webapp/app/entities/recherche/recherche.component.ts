...
export class RechercheComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.rechercheService
      ...
      .subscribe(
        (res: HttpResponse<IRecherche[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IRecherche): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInRecherches(): void {
    this.eventSubscriber = this.eventManager.subscribe('rechercheListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IRecherche[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
